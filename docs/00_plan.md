...
| T0.8f | Red | `tests/integration/test_es_resource_drift.py` — parse `resources/es/chunks_v1.json` and the JSON block in spec §5.2; assert deep-equal (B26). Prevents prose/resource drift. Pure unit-style file diff; no testcontainer needed. | T0.8e | [ ] | QA | W2 |
...
| T0.8g | Red | `tests/integration/test_es_plugin_required.py` — boot against an ES cluster lacking `analysis-icu` plugin → `/readyz` returns 503 with `error_code=ES_PLUGIN_MISSING` listing `analysis-icu`; init refuses to `PUT` the index (B26, I5). | T0.8d, T0.9 | [ ] | QA | W2 |
| T0.10 | Structural | `src/ragent/bootstrap/broker.py` — TaskIQ `broker` instance (module-level export). Dispatches on `REDIS_MODE` (B27/B28): standalone reads `REDIS_BROKER_URL`; sentinel uses `REDIS_SENTINEL_HOSTS` + `REDIS_BROKER_SENTINEL_MASTER`. Single canonical import path `from ragent.bootstrap.broker import broker` used by every `@broker.task` decorator (T3.2b, T3.2d) and by the worker entrypoint (T7.5d). | T0.1 | [ ] | Dev | W2 |
| T0.10a | Red | `tests/unit/test_broker_topology.py` — with `REDIS_MODE=standalone` the broker module constructs against `REDIS_BROKER_URL`; with `REDIS_MODE=sentinel` it constructs against `REDIS_SENTINEL_HOSTS` + `REDIS_BROKER_SENTINEL_MASTER` (B27); missing var in sentinel mode → SystemExit. | T0.10 | [ ] | QA | W2 |
| T0.11 | Structural | `.env.example` at repo root enumerating **every** variable from spec §4.6 in the same subsection order (bootstrap → datastore → redis → clients → worker → pipeline → timeouts → observability). Required vars carry `# REQUIRED — set before boot`; default-overridable vars show their default value commented in. Operator workflow: `cp .env.example .env` → fill required → start API + worker. | T0.1 | [ ] | Dev | W2 |
| T0.11a | Red | `tests/unit/test_env_example_drift.py` — parse spec §4.6 tables and `.env.example`; assert (a) symmetric variable set (no var in spec missing from example, no orphan in example), (b) defaults match where declared, (c) every `(required)` var in spec is marked `# REQUIRED` in example. CI gate against operator-config drift (B30). | T0.11 | [ ] | QA | W2 |
...
| T7.5a | Structural | `src/ragent/bootstrap/composition.py` — **composition root (B30)**: builds singletons `TokenManager`, `EmbeddingClient`, `LLMClient`, `RerankClient`, `MinIOClient`, `Elasticsearch`, MariaDB engine; instantiates `DocumentRepository`, `ChunkRepository`; constructs plugin instances with B17 DI args (`VectorExtractor(repo, chunks, embedder, es)`, `StubGraphExtractor()`); registers all required + optional plugins onto a single `PluginRegistry`; builds the chat pipeline factory parameterised by `CHAT_JOIN_MODE` (C6). Exposes a `Container` dataclass + `get_container()` accessor used by FastAPI `Depends` (T2.14, T3.10, T3.12, T7.8) and by worker tasks (T3.2b, T3.2d) and Reconciler (T5.2). All env-var reads happen here, not inside business modules. | T1.7, T1.12, T2.2, T2.4, T2.6, T3.6, T3.8, T4.2, T4.4, T4.6, T4.8 | [ ] | Dev | W6 |
