...
| T-CAT.W3 | Behavioral | • **Achieve:** Pin the attachment-MIME→artifact-`content_type` relationship as a single registry instead of leaving it implicit. `ARTIFACT_CONTENT_TYPE` (OCP — a dict, not branching) is the one place this is decided; `ASTCipher.encrypt_ast()` now requires `content_type` and stores it in the encrypted envelope's `metadata` (not a DB column — never queried, only a render hint, so no migration). Scoped down from a fuller "universal artifact table" redesign (variant/content_type split) per explicit user direction — DB schema unchanged, `chat_attachment_artifacts` still only stores `ast_type`.<br>• **Deliver:** `src/ragent/schemas/attachments.py` (`ARTIFACT_CONTENT_TYPE`); `src/ragent/security/ast_cipher.py` (`content_type` param); `src/ragent/services/chat_attachment_service.py` (wires it through `process()`); `docs/spec/chat_attachments.md` §2.1/§5/§10 (mapping table + DDL); `docs/00_domain_map.md`; `tests/unit/test_attachments_schema.py`, `tests/unit/test_ast_cipher.py`, `tests/unit/test_chat_attachment_service.py`. | TBD | [x] | Dev |
