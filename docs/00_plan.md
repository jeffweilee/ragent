...
| T0.8 | Structural | DB migration `migrations/001_initial.sql` — documents (incl. `create_user VARCHAR(64) NOT NULL` + `object_key VARCHAR(256) NOT NULL` per B14, B25) + chunks tables, indexes `idx_status_updated`, `idx_source_app_id_status_created`, `idx_create_user_document`. Alembic configured (B3). | T0.1 | [ ] | Dev | W2 |
