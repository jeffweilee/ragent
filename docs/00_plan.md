...
| T7.5 | Structural | Startup guard: `src/ragent/bootstrap/app.py` refuses to start unless `RAGENT_AUTH_DISABLED=true AND RAGENT_ENV=dev` (B28); reads `RAGENT_HOST` (B28, default `127.0.0.1`) — guard refuses any non-loopback bind while in P1 OPEN — and `RAGENT_PORT` (B28, default 8000) for the uvicorn entrypoint. Configures `LOG_LEVEL` (B28, default `INFO`) for app + TaskIQ + Reconciler logging. | T2.14 | [ ] | SRE | W6 |
