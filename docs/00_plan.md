...
| T2.6 | Green | `src/ragent/storage/minio_client.py` (B10 key format, B25 returns key string; per-call timeouts wired from `MINIO_PUT_TIMEOUT_SECONDS` / `MINIO_GET_TIMEOUT_SECONDS`, B28). | T2.5 | [x] | Dev | W3 |
| T2.7 | Red  | `tests/unit/test_ingest_service_create.py` — validate `source_id`+`source_app`+`source_title` mandatory (S23 → 422 on missing/empty — B11), MIME validate against C1 allow-list `{text/plain, text/markdown, text/html, text/csv}` (≤50 MB) → put → repo.create (persists `source_id`, `source_app`, `source_title`, `object_key` per B25, `create_user`, optional `source_workspace`) → kiq dispatch; rolls back row if MinIO put fails. | T2.2, T2.6, T1.7 | [x] | QA | W3 |
| T2.8 | Green | `src/ragent/services/ingest_service.py::create` (≤ 30 LOC/method). | T2.7 | [x] | Dev | W3 |
| T2.9 | Red  | `tests/unit/test_ingest_service_delete.py` — cascade order (acquire NOWAIT → DELETING short tx → fan_out_delete OUTSIDE tx → chunks → MinIO if UPLOADED/PENDING → row); on any failure row stays DELETING (S13); idempotent re-delete returns 204 (S14); fan_out_delete runs with no DB tx open (P-E). | T2.8, T1.8 | [x] | QA | W3 |
| T2.10 | Green | `src/ragent/services/ingest_service.py::delete`. | T2.9 | [x] | Dev | W3 |
| T2.11 | Red | `tests/unit/test_ingest_service_list.py` — cursor pagination by `document_id` ASC; `?limit` honours upper bound `INGEST_LIST_MAX_LIMIT` (B28, default 100) — request with `limit=200` is clamped or 422 per service contract; `next_cursor` correctness (S15, P1 OPEN: no ACL filter). | T2.2 | [x] | QA | W3 |
| T2.12 | Green | `src/ragent/services/ingest_service.py::list`. | T2.11 | [x] | Dev | W3 |
| T2.13 | Red | `tests/unit/test_ingest_router.py` — Router only parses/validates and delegates; 415 (`error_code=INGEST_MIME_UNSUPPORTED`) on MIME outside C1 allow-list, 413 (`INGEST_FILE_TOO_LARGE`) on body > `INGEST_MAX_FILE_SIZE_BYTES` (B28, default 50 MB / 52428800); 422 on missing/empty `source_id` / `source_app` / `source_title` (S23, B11); `X-User-Id` required (P1 OPEN). All non-2xx bodies are RFC 9457 `application/problem+json` with `error_code` (B5); 422 carries `errors[]`. | T2.8, T2.10, T2.12 | [x] | QA | W3 |
| T2.14 | Green | `src/ragent/routers/ingest.py` + `src/ragent/errors/problem.py` (RFC 9457 builder, B5). Declares all endpoints in spec §4.1. | T2.13 | [x] | Dev | W3 |

### Track T3 — Pipelines (Ingest + Chat assembly)

| # | Category | Task | Depends On | Status | Owner | Week |
|---|---|---|---|:---:|---|:---:|
| T3.1 | Red | `tests/integration/test_ingest_pipeline.py` — Haystack `Convert→Clean→LanguageRouter→{cjk_splitter\|en_splitter} (sentence)→Embed` (B1); mock embedder. EN doc routed to `en_splitter`; CJK doc routed to `cjk_splitter`; chunk count == sentence count. | T2.4, T4.2 | [x] | QA | W3 |
| T3.2 | Green | `src/ragent/pipelines/factory.py` + `pipelines/ingest.py`. | T3.1 | [x] | Dev | W3 |
| T3.2a | Red | `tests/integration/test_worker_minio_cleanup.py` — terminal status (`READY` or `FAILED`) is committed **before** `MinIOClient.delete_object` is called; if the delete raises, the document still ends up in the terminal status and an `event=minio.orphan_object` log is emitted (S16, S21). On retry path (still `PENDING`), MinIO object is retained. | T3.2 | [x] | QA | W3 |
...
| T3.2i | Red | `tests/integration/test_worker_heartbeat.py` — Given a worker is mid-pipeline (mock 4-min sleep), When **a stand-in Reconciler query** runs at 5 min wall-clock (`list_pending_stale(now - RECONCILER_PENDING_STALE_SECONDS)`, B28), Then it observes `updated_at < 30 s` ago and the worker row is **not** in the result set (S33, B16). When the worker is killed, `updated_at` ages past 5 min and the same query returns the row exactly once. (Full Reconciler entrypoint behaviour covered in T5.) | T3.2b, T2.2 | [x] | QA | W3 |
| T3.2j | Red | `tests/integration/test_pipeline_timeout.py` — Given a pipeline body that sleeps > `PIPELINE_TIMEOUT_SECONDS`, When the timeout fires, Then row → `FAILED` with `error_code=PIPELINE_TIMEOUT`, cleanup ran, `event=ingest.failed reason=pipeline_timeout` logged (S34, B18). | T3.2b | [x] | QA | W3 |
| T3.2k | Red | `tests/integration/test_csv_row_merger.py` — 10 000-row CSV (~50 chars each); after ingest, `chunks` row count ≤ `ceil(total_chars / CSV_CHUNK_TARGET_CHARS)` (≈ 250); a `.txt` of identical size produces sentence-level chunks (RowMerger bypassed) — verifies `ConditionalRouter` on MIME (S35, B24). | T3.2 | [x] | QA | W3 |
| T3.2l | Green | Pipeline factory adds `RowMerger` SuperComponent on the `text/csv` branch only (`ConditionalRouter` keyed on MIME); merges rows joined by `\n` until buffer ≥ `CSV_CHUNK_TARGET_CHARS` (B24). | T3.2k | [x] | Dev | W3 |
| T3.2c | Red | `tests/integration/test_supersede_task.py` — on `READY`, kiq `ingest.supersede`; task pops oldest loser one-at-a-time and commits per-loser (P-C, S31), keeps `MAX(created_at)`, cascade-deletes the rest (S17); same `source_id` with different `source_app` coexists (S22); out-of-order finish still converges to MAX(created_at) survivor (S20); FAILED never enqueues supersede (S18); idempotent re-run (S19). | T3.2b, T2.10 | [x] | QA | W3 |
| T3.2d | Green | `src/ragent/services/ingest_service.py::supersede(document_id)` + TaskIQ `@broker.task("ingest.supersede")` worker (broker from T0.10); loops `pop_oldest_loser_for_supersede` + cascade delete + commit; never holds K row locks across K cascades. | T3.2c, T0.10 | [x] | Dev | W3 |
| T3.2e | Red | `tests/integration/test_pipeline_retry_idempotent.py` — Reconciler retry of a partially-written ingest produces no duplicate chunks; pipeline first step is `delete_by_document_id` + `VectorExtractor.delete` (R4, S25). | T3.2 | [x] | QA | W3 |
| T3.2f | Green | Pipeline factory prepends idempotency-clean step. | T3.2e | [x] | Dev | W3 |
| T3.2g | Red | `tests/unit/test_worker_acquire_nowait.py` — concurrent dispatch: second worker fails fast on `FOR UPDATE NOWAIT`, re-kiqs self with backoff, does **not** increment `attempt` (R7, S28). | T3.2 | [x] | QA | W3 |
| T3.2h | Green | Worker uses `acquire_nowait`; on `LockNotAvailable` exception, re-kiq with exponential backoff (cap 30 s). | T3.2g | [x] | Dev | W3 |
| T3.3 | Red | `tests/unit/test_chat_request_schema.py` — request validation (B12, S6c): `messages` required (422 `error_code=CHAT_MESSAGES_MISSING` if missing/empty); defaults from env vars (`RAGENT_DEFAULT_LLM_PROVIDER/MODEL/TEMPERATURE/MAX_TOKENS`); `provider` validated against allow-list `{"openai"}` else 422 `CHAT_PROVIDER_UNSUPPORTED` (B22); auto-prepend `{"role":"system","content":"<RAGENT_DEFAULT_SYSTEM_PROMPT>"}` if no system entry (S6b, observable via mock LLM capture). **Filter fields (B29, S6i):** `source_app` / `source_workspace` are optional; both omitted ⇒ no filter; empty string or > 64 chars ⇒ 422 `CHAT_FILTER_INVALID`. | T4.6 | [x] | QA | W4 |
| T3.4 | Green | `src/ragent/schemas/chat.py` (Pydantic) — `ChatRequest` with env-driven defaults + `provider` allow-list validator (B22) + optional `source_app`/`source_workspace` filter fields (B29, length 1–64, snake_case per B21); `Message`; `normalize_messages()` helper that prepends default system. JSON field naming per B21 (`maxTokens` camelCase; ID/scope fields snake_case). | T3.3 | [x] | Dev | W4 |
| T3.5 | Red | `tests/integration/test_chat_pipeline_retrieval.py` — QueryEmbedder (timeout `EMBEDDER_QUERY_TIMEOUT_SECONDS`, B28) → {ESVector kNN on `embedding` (`bbq_hnsw`, B26) + ESBM25 `multi_match(["text","title^2"])` with `icu_text` analyzer (B26) sequential, B15} both retrievers timeout `ES_QUERY_TIMEOUT_SECONDS` (B28, default 10) → `DocumentJoiner` configured by `CHAT_JOIN_MODE` env (C6) → `SourceHydrator` joins `documents` for `(source_app, source_id, source_title)` (B11) and **truncates `excerpt` to `EXCERPT_MAX_CHARS`** (B23) → returns hydrated chunks. Title-only lexical query recalls via BM25 `title^2`; semantic title query (synonym of title) recalls via the title-aware `embedding` (B15). **CJK BM25 (S36, B26):** Chinese-text query (e.g. `"產品規格"` against a doc body containing the same term with no whitespace) recalls via `icu_tokenizer`; the same query against a `standard`-analyzed control index does not — proves the analyzer choice matters. **Filter (B29, S6f/S6g/S6h):** when `ChatRequest.source_app` is set, both retrievers carry `term: {source_app: <v>}` in their `filter` clause and only matching chunks reach the joiner; combined `source_app + source_workspace` ANDs; non-matching filter ⇒ empty hydrated list (router maps to `sources: null`, S6h). Empty index → hydrated list is empty. | T2.4, T4.2, T4.4, T2.2, T1.12 | [x] | QA | W4 |
| T3.5a | Red | `tests/unit/test_pipeline_factory_join_mode.py` — for each value of `CHAT_JOIN_MODE` ∈ {`rrf`, `concatenate`, `vector_only`, `bm25_only`} the factory returns a pipeline with the expected component graph (e.g. `vector_only` has no BM25 retriever and no joiner). Default is `rrf` (C6). | T3.5 | [x] | QA | W4 |
...
| T3.14 | Green | `src/ragent/clients/rate_limiter.py` — `RateLimiter` adapter; reads `REDIS_RATELIMIT_URL` (standalone) or `REDIS_RATELIMIT_SENTINEL_MASTER` + `REDIS_SENTINEL_HOSTS` (sentinel) from composition root (T7.5a); key prefix `ratelimit:`. Composition root exports `chat_rate_limit_dep` FastAPI `Depends` factory bound to `CHAT_RATE_LIMIT_PER_MINUTE` / `CHAT_RATE_LIMIT_WINDOW_SECONDS` (B31). | T3.13 | [x] | Dev | W4 |
| T3.15 | Red | `tests/integration/test_chat_rate_limit.py` — with `CHAT_RATE_LIMIT_PER_MINUTE=N`, **N+1** `POST /chat` calls from same `X-User-Id` within the window: first N return 200, the (N+1)th returns 429 `application/problem+json` with `error_code=CHAT_RATE_LIMITED` + `Retry-After` header (seconds until window reset, B31, S37); same applies to `/chat/stream`. Different `X-User-Id` gets a fresh budget (per-user isolation). After window expiry (`time.sleep(window+ε)` against fake clock), counter resets. | T3.14, T3.10, T3.12 | [x] | QA | W4 |
| T3.16 | Green | Wire `Depends(chat_rate_limit_dep)` onto the chat router (T3.10/T3.12) — applies to both `/chat` and `/chat/stream`. Health endpoints, ingest, and MCP are NOT rate-limited (router-level dependency, not global middleware, B31). | T3.15 | [x] | Dev | W4 |
| T3.17 | Red | `tests/unit/test_build_rag_messages.py` + extensions to `tests/integration/test_chat_endpoint.py` and `test_chat_stream_endpoint.py` — when retrieval returns ≥1 doc, the LLM receives (a) a leading system message containing the multi-intent grounded template and (b) the last user message wrapped with `=== CONTEXT START ===` … `=== CONTEXT END ===` followed by the original query verbatim. Each rendered chunk includes `source_app`, `source_title`, `document_id`, and excerpt. With user-supplied system message, grounding-rules are prepended as a SEPARATE system message preceding the user's. Empty retrieval ⇒ identical to T3.3 default-system path; user message NOT wrapped. | T3.10, T3.12 | [x] | QA  | W4 |
| T3.18 | Green | `src/ragent/schemas/chat.py::build_rag_messages` + `_render_context` + `_wrap_last_user`; routers `/chat` and `/chat/stream` swap `normalize_messages` for `build_rag_messages` and run retrieval before message construction. New env vars `RAGENT_DEFAULT_RAG_SYSTEM_PROMPT` and `RAGENT_RAG_GROUNDING_RULES` (defaults wired; `.env.example` updated). | T3.17 | [x] | Dev | W4 |

### Track T4 — Third-Party Clients

| # | Category | Task | Depends On | Status | Owner | Week |
|---|---|---|---|:---:|---|:---:|
| T4.1 | Red | `tests/unit/test_token_manager.py` — POSTs `AI_API_CLIENT_ID` + `AI_API_CLIENT_SECRET` (B28) to `AI_API_AUTH_URL/auth/api/accesstoken`; refreshes at `expiresAt − 5min` boundary using fake clock (S9); single-flight refresh: 100 concurrent calls share one HTTP exchange (P-F). Credentials never appear in log records or error responses. | T0.6 | [x] | QA | W4 |
| T4.2 | Green | `src/ragent/clients/auth.py` (`TokenManager`) with `asyncio.Lock` / `threading.Lock` around exchange. | T4.1 | [x] | Dev | W4 |
| T4.3 | Red | `tests/unit/test_embedding_client.py` — POST shape, `bge-m3`, validates `returnCode == 96200`, retry 3× @ 1 s; **batch interface** accepts `list[str]` and issues one HTTP call per batch up to `EMBEDDER_BATCH_SIZE` (B28, default 32, P-B); ingest call bound by `EMBEDDER_INGEST_TIMEOUT_SECONDS` (B28, default 30); single-string query call bound by `EMBEDDER_QUERY_TIMEOUT_SECONDS` (B28, default 10) — asymmetric per C8. | T4.2 | [x] | QA | W4 |
| T4.4 | Green | `src/ragent/clients/embedding.py` (batch size from `EMBEDDER_BATCH_SIZE`, two timeout call paths from `EMBEDDER_INGEST_TIMEOUT_SECONDS` / `EMBEDDER_QUERY_TIMEOUT_SECONDS`). | T4.3 | [x] | Dev | W4 |
| T4.5 | Red | `tests/unit/test_llm_client.py` — streaming async iterator yields deltas; timeout `LLM_TIMEOUT_SECONDS` (B28, default 120); retry 3× @ 2 s. | T4.2 | [x] | QA | W4 |
| T4.6 | Green | `src/ragent/clients/llm.py` (timeout from `LLM_TIMEOUT_SECONDS`). | T4.5 | [x] | Dev | W4 |
| T4.7 | Red | `tests/unit/test_rerank_client.py` — POST shape, `bge-reranker-base`, `top_k=2`. (Wired P2.) | T4.2 | [x] | QA | W4 |
| T4.8 | Green | `src/ragent/clients/rerank.py`. | T4.7 | [x] | Dev | W4 |
