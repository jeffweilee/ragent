...
| # | Category | Task | Status | Owner |
|---|---|---|:---:|---|
| T-MC.1 | Red+Green | • **Achieve:** Guard against `_IncludedRouter` `AttributeError`: (a) `prometheus-fastapi-instrumentator>=8.0.0` requires `starlette>=1.0.0` which pulls in incompatible packages; (b) `fastapi>=0.137.0` introduces `_IncludedRouter` (no `.path`) into `app.routes` regardless of starlette version — both caps are required. `_get_route_name` accesses `route.path` unconditionally, crashing every first request.<br>• **Deliver:** Pins `prometheus-fastapi-instrumentator>=7.0.0,<8.0.0` and `fastapi>=0.115.0,<0.137.0` in `pyproject.toml`; `_load_pyproject()` helper with Python <3.11 skip guard; tests `test_pyproject_pins_instrumentator_below_v8`, `test_pyproject_pins_fastapi_below_0137`, `test_get_route_name_raises_on_pathless_route`.<br>• **Success criteria:** `uv run pytest tests/unit/test_http_metrics.py` green (6 tests); both pins present in `pyproject.toml`. | [x] | Dev |
