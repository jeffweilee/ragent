...
| `BRAIN_FORWARD_HEADERS`               | (optional)       | Comma-separated allowlist of **inbound** header names ragent copies verbatim onto every outbound brain call (run + reconnect-producer + cancel + `/upstream/*` proxy) so brain can relay them to on-behalf-of downstreams. Typically `X-Auth-Token` (the raw JWT). ragent stays the verification boundary; these are carried, not re-verified. The service-owned `X-User-Id` / `X-Brain-Key` always win over any same-named forwarded value. Unset = nothing forwarded (current behaviour). |
...
| `RAGENT_KEK_BASE64`                   | (optional)       | T-CAT — base64 Key-Encryption-Key (32 bytes), injected at process start. When set (together with `RAGENT_ENCRYPTED_DEK_BASE64`), the composition root constructs `KeyManager`/`ASTCipher` and wires the chat-attachment subsystem (`POST/GET /chatagent/v3/attachments`, `ChatAttachmentService`, `DocumentArtifactResolver`). When unset, attachment routes are not registered and `/chatagent/v3` chat never folds an `<attachments>` block. **Never logged, never echoed.** |
| `RAGENT_ENCRYPTED_DEK_BASE64`         | (required when `RAGENT_KEK_BASE64` set) | base64 Data-Encryption-Key, AES-Key-Wrapped under the KEK. The composition root reads it and constructs `KeyManager`, which unwraps it once at boot; the unwrapped DEK is held in memory for the process lifetime and used by `ASTCipher` (AES-256-GCM) to encrypt/decrypt attachment AST artifacts. **Never logged, never echoed.** |
...
| `ATTACHMENT_ARTIFACT_MAX_CHARS`       | `10000`          | Context-window budget gate: `DocumentArtifactResolver` selects the `complete` artifact variant only when its (pre-computed) `char_count` is at or under this; otherwise falls back to `simplified`. Also a hard per-attachment truncation cap on whichever variant is selected. No decrypt needed to make the selection decision (T-CAT.W16). |
| `ATTACHMENT_TOTAL_MAX_CHARS`          | `50000`          | Per-turn aggregate cap on the sum of injected `content` across all resolved attachments (processed in `attachmentIds` order); bounds worst case from `ATTACHMENT_MAX_FILES × ATTACHMENT_ARTIFACT_MAX_CHARS`. `DocumentArtifactResolver` truncates/omits `content` once spent and logs `document_artifact_resolver.attachment_content_truncated` (T-CAT.W20). |
...
| `INGEST_PDF_USE_OCR`                  | `false`          | Enable selective OCR for scanned PDFs. When `false` (default), only the text layer is extracted — text PDFs complete in seconds with no OCR inference. When `true`, each page is pre-scanned via `get_text()` and pages with fewer than `INGEST_PDF_OCR_CHAR_THRESHOLD` extractable characters are processed through RapidOCR. |
| `INGEST_PDF_OCR_CHAR_THRESHOLD`       | `50`             | Minimum extractable character count for a PDF page to be treated as a text page. Pages below this threshold are classified as scanned (image-only) and OCR'd when `INGEST_PDF_USE_OCR=true`. |
| `INGEST_PDF_OCR_MAX_SCANNED_PAGES`    | `10`             | Maximum number of scanned pages allowed per document when `INGEST_PDF_USE_OCR=true`. If the pre-scan finds more scanned pages than this cap, the task is rejected immediately with `INGEST_PDF_OCR_PAGES_EXCEEDED` on `documents.error_code`. Keeps per-document OCR time bounded (≤ 10 pages × ~6 s/page at 150 DPI ≈ 60 s). |
| `INGEST_PDF_OCR_DPI`                  | `150`            | DPI used when rendering a page pixmap for RapidOCR. 150 produces the same detection quality as 300 (RapidOCR Det resizes internally to 736 px regardless) while making `fitz.Page.get_pixmap()` ~4× faster. |
| `INGEST_PDF_OCR_THREADS`              | `2`              | `intra_op_num_threads` passed to the `RapidOCR` ONNX engine singleton at worker startup when `INGEST_PDF_USE_OCR=true`. Higher values trade CPU for per-page OCR latency. Has no effect when `INGEST_PDF_USE_OCR=false`. |
...
| `REDIS_PAT_LOCK_TTL_SECONDS`          | `45`             | TTL of the per-nt refresh lock (bounds a crashed refresh holder). Must exceed `PAT_REFRESH_TIMEOUT_SECONDS` so the holder keeps its lock through the call; release is owner-checked so an expiry-then-reacquire never cross-deletes. Since T-RETRY.3 a refresh is one call, so 45 > 30 covers the **worst** case, not just the typical one — under the old retry budget (~123 s) the lock expired mid-refresh exactly when the refresh service was degraded, i.e. when serialising mattered most. |
