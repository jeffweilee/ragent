...
| `RAGENT_ENCRYPTED_DEK_BASE64`         | (required when `RAGENT_KEK_BASE64` set) | base64 Data-Encryption-Key, AES-Key-Wrapped under the KEK. `KeyManager.from_env()` unwraps it once at boot; the unwrapped DEK is held in memory for the process lifetime and used by `ASTCipher` (AES-256-GCM) to encrypt/decrypt attachment AST artifacts. **Never logged, never echoed.** |
