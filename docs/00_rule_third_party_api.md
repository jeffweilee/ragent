...
```json
{
  "metadata": {
    ...
    "user": "<user_id resolved via RAGENT_JWT_CLAIM_USER_ID claim — same source as all other endpoints>",
...
> Set `"stream": true` to receive an SSE stream instead of a single JSON response — all other request fields are identical.

> **Human-in-the-loop resume:** to continue a run paused by an interrupt
> (`humanInTheLoopMeta.isInterrupt`), send `inputData` as
> `{"lastMessageId": "<interrupt messageId>", "message": ""}` (same `metadata`,
> `stream` as a normal turn). `lastMessageId` is the upstream `messageId` the run
> interrupted on. The upstream supports go / no-go only — there is no payload
> field for a structured human response.
...
data: {
  ...
  "returnData": {
    ...
    "messages": [
      {
        "role": "assistant|tool",
        "content": "<full or initial content>",
        "messageId": "<id>",
        "displayMeta": {
...
**Subsequent delta events** — same shape; `returnData.messages[].content` carries the incremental text delta only.
...
> **Nullable / optional fields:** `humanInTheLoopMeta` may be absent. `finish_reason` and `tool_calls` are present only when the message ends a tool-call turn.
>
> **`messageMeta.langgraph_node` enum:** `planner` | `commander` | `summarizer`.
>
