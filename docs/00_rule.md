...
- **Rule: State-Machine Row Locking — atomic conditional UPDATE, never `FOR UPDATE`.**
    - **Action**: Single-row state transitions (claim, status update, promote/demote) MUST use an atomic conditional `UPDATE … WHERE status IN (...)` dispatched on `rowcount` — never `SELECT … FOR UPDATE [NOWAIT]`. A redispatched `PENDING → PENDING` claim is a normal `rowcount=1` outcome, not an illegal transition.
    - **Reserved for bulk scans only**: `SKIP LOCKED` remains legitimate for reconciler sweeps and supersede scans over many candidate rows — never for single-row transitions.
    - **Rationale**: `FOR UPDATE` held across a long pipeline body blocks `SKIP LOCKED` reconciler sweeps and silently masks a hung (not dead) worker (SRE journal 2026-05-14 "Recovery semantics closeout" — supersedes the earlier `FOR UPDATE`-based prescription).
    - **Verification**: any new `FOR UPDATE [NOWAIT]` on a single-row `documents`-style state-machine path is a spec drift bug — reject in review.
...
- **Action**: (a) explicit connect + socket timeouts on **every** client, including Sentinel `sentinel_kwargs` (discovery hangs during failover too); (b) never apply a read timeout to a connection parked in a blocking command (e.g. `brpop()`) — it raises `TimeoutError`, not `ConnectionError`, silently killing a consumer whose handler only catches the latter; give blocking-command connections connect-only timeouts; (c) a **per-client circuit breaker** on request/response clients (`PatCache`, `RateLimiter`, `ChatStreamStore`) so a sustained outage costs one open/half-open cycle, not a per-request timeout tax, excluding healthy-server conflict signals (e.g. `redis.WatchError`) from the failure count — a client whose degraded value is "stay queued" rather than "answer degraded" (the TaskIQ broker) defers dispatch instead of breaking, since a breaker there would still need to report *some* outcome per call; (d) a **documented degraded value per operation** (allow / miss / no-op / defer) — never let it propagate as a 5xx on a path that doesn't need it.
- **Verification**: bounded-timeout test per topology; a breaker-open test asserting zero I/O for each breaker-backed client. Source: SRE journal 2026-08-10 "Weak-Dep Latency Bound"; landed in Track T-RG (`docs/00_plan_done.md`).
...
- **Rule: Integration/e2e seed payloads and ES fixtures must match the live schema/mapping.** When a Pydantic schema or `resources/es/<index>.json` mapping gains a required field or renames one, every seed dict/fixture constructing that shape MUST be updated in the same commit. Recurred 3× (missing `index_name`, missing `source_id`/`source_app`, fixture key `"content"` vs real `"text"` masking a real bug). Audit: diff fixture/seed builders against the schema/mapping whenever either changes.
