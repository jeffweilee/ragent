...
**Realtime status (NATS, not an HTTP route):** when `NATS_SERVERS` is set, ragent publishes status transitions to the per-user subject `session.<user_id>.status` — `{session, running:true}` (run starts), `{session, running:false, hasNewReply:true}` (finishes), `{session, hasNewReply:false}` (read). The frontend subscribes over its own NATS connection and merges deltas onto the `sessionList` snapshot. NATS core pub/sub is lossy, so the client **must re-fetch `sessionList` on (re)connect/error** to re-sync (the snapshot is the durable truth; the publish is best-effort). NATS unset → list is snapshot-only.
