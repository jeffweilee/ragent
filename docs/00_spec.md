...
1. **Bounded socket budget.** `redis-py` defaults `socket_timeout` / `socket_connect_timeout` to `None`. Against a *refusing* Redis that is harmless, but against a *blackholed* one (master mid-failover, dropped packets) an unbounded connect blocks until the kernel gives up (`tcp_syn_retries=6` ≈ 127 s) — and these clients are called from the event loop, so it stalls every other in-flight request. `REDIS_CONNECT_TIMEOUT_SECONDS` (default `0.25`) and `REDIS_SOCKET_TIMEOUT_SECONDS` (default `1`) apply to **every** client and to the TaskIQ broker, in standalone and Sentinel mode. In Sentinel mode the budget MUST also reach `sentinel_kwargs` — master discovery runs over its own connections, and an unbounded discovery hangs in exactly the failover this is meant to survive.
