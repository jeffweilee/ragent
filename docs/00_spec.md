...
>    ├ text/csv      → _CsvASTSplitter (one atom per row group; §4.2)
...
> Full spec: [docs/spec/resilience.md](spec/resilience.md) — reconciler recovery (B9), multi-READY repair, Redis weak-dependency mechanisms (bounded socket budget, per-client circuit breaker, degraded-value contract per op, T-RG), ingest-dispatch/ES-bootstrap degradation, and the §3.6.1 chaos drill matrix (B49).
