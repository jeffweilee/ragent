...
| `HTTP_ERROR_LOG_MAX_BYTES`            | `8192`           | Max bytes of request/response body included in `http.upstream_error` log records. Bodies above this size are truncated with `request_truncated` / `response_truncated` set to `true`. Auth headers (`Authorization`, `apikey`, `Cookie`) and the J1 `key` field of the auth POST are always redacted regardless of size. |
