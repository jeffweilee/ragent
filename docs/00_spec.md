...
Cardinality: labels enumerate from closed enums (`phase`, `outcome`) or deployment-bounded sources (`system`, `tool` ← yaml-declared). Caller helpers guard against typos by collapsing unknown enum values to a safe default. Load failures appear simultaneously as structured `mcp_hub.load_failure` log events (fields: `source`, `reason`, `system`, `phase`, `tool`) so log-based debugging matches metric drill-down dimensions.
...
| `MINIO_SITES`                         | (required)       | v2: JSON list of `{name, endpoint, access_key, secret_key, bucket, secure?, read_only?}`. Must include `name="__default__"` (inline ingest). Supersedes legacy `MINIO_ENDPOINT/ACCESS_KEY/SECRET_KEY/SECURE/BUCKET` (all removed in C6). |
...
**Topology:** 1 shard, 0 replicas (dev/CI); prod overrides via cluster template. `icu_text` analyzer (`icu_tokenizer` + `icu_folding` + `lowercase`) required for CJK BM25 — `analysis-icu` plugin is a hard ES dependency (B26). Test override: `tests/resources/es/chunks_v1.json` uses `standard` analyzer + `flat` kNN so vanilla CI containers work (B42); `bbq_hnsw` (B58) saves ~32× heap vs `flat` at negligible recall cost.

**Default ingest pipeline `chunks_default` (B59):** `settings.index.default_pipeline = "chunks_default"` fills `indexed_at` from the ES coordinating-node clock on every write — no Python writer touches the field. Source of truth: `resources/es/pipelines/chunks_default.json`, mirrored below.
...
**Bootstrap order (B59):** `init_es()` MUST PUT the pipeline BEFORE the index — ES rejects index creation whose `default_pipeline` references a missing pipeline (invariant pinned by `test_init_schema.py`). `indexed_at` reflects **last** write time under `DuplicatePolicy.OVERWRITE`; `set { override: false }` is a no-op here. Operators needing first-write time read `documents.created_at` (MariaDB SoT).
