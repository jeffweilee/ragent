...
**Storage model:** chunks live **only** in ES `chunks_v1`. The MariaDB `chunks` table is **dropped**. `documents` keeps metadata. Two stores total: `documents` (MariaDB, metadata) + `chunks_v1` (ES, content + embedding + raw_content).

**Per-step structured logging:** every pipeline component emits `event=ingest.step.{started,ok,failed}` with `{document_id, step, mime_type, duration_ms, error_code?, error?}`. Failures map to a small enum (`PIPELINE_UNROUTABLE`, `EMBEDDER_ERROR`, `ES_WRITE_ERROR`, `PIPELINE_TIMEOUT`). Happy path emits `event=ingest.ready` with chunk count.
...
- **S12/S14** DELETE cascade / idempotent re-DELETE — `READY` doc: `DELETING` → plugins called once → ES/row cleared → 204 (MinIO already cleared). Re-DELETE: 204, no plugin calls.
- **S13/S16** mid-delete failure / MinIO cleanup — Mid-cascade failure → row stays `DELETING`; Reconciler resumes ≤ 5 min. Terminal state (`READY`/`FAILED`) → MinIO object deleted; re-processing needs re-upload.
...
- **S24/S25 UPLOADED orphan / no duplicate chunks (R1/R4)** — UPLOADED for > 5 min (message lost/broker outage) → Reconciler re-kiqs; doc proceeds normally. Reconciler retry → chunks count = chunker output; no ES orphans.
- **S26/S27 multi-READY repair / FAILED cleanup (R3/R5)** — Two READY rows for same `(source_id, source_app)` → Reconciler re-enqueues supersede; converges in one cycle. FAILED commit → chunks + ES `chunks_v1` cleared (no chat retrieval leakage).
...
- **S30/S31 reconciler heartbeat + single-loser-per-tx (R8/P-C)** — Tick: increments `reconciler_tick_total`, emits `event=reconciler.tick`; absence > 10 min triggers alert. Supersede K=10 losers: each deleted in its own committed tx.
...
**BDD:**
- **S4/S11 Protocol conformance / duplicate registration** — Missing any of `name`/`required`/`queue`/`extract`/`delete`/`health` → `isinstance` returns `False`, `register()` raises. Second `register()` with same name → `DuplicatePluginError`; existing instance unaffected.
...
- **S6b/S6c** — Omitted `role:"system"` → server prepends default system message (observable via mock LLM capture). Request with only `messages` → `provider="openai"`, `model="gptoss-120b"`, `temperature=0.7`, `maxTokens=4096` defaults applied.
- **S6d/S6h** — Empty retrieval (empty index, retriever error, or no filter-matching chunks) → `sources: null`; LLM still answers.
...
- **S6f/S6g filter (B29→B35)** — `source_app` supplied → both retrievers apply `term` filter; `source_app` + `source_meta` both supplied → AND filter; `sources[]` contains only matching rows.
...
**BDD:**
- **S38/S40 retrieve `chunks[]` shape** — `dedupe=false` (default): multiple chunks from the same `document_id` all appear in `chunks[]`. Empty index: `{"chunks":[]}` returned (never `null`).
...
| **C2/C3** | MariaDB-ES split / ES bulk partial | (a) Worker `ConnectionError` between DB commit and ES bulk; (b) `_bulk` returns `errors:true`, 5/50 failed | (a) Worker retries idempotently; READY, all chunks present. (b) Worker retries failed items only (OVERWRITE); READY all 50; `event=es.bulk_partial_failure` |
| **C4/C5** | Rerank 5xx / LLM stream interrupt | (a) WireMock `/rerank` 500 for 3 calls; (b) WireMock streams 3 deltas then drops TCP | (a) Chat 200 with RRF-ordered sources (fail-open); `rerank_degraded_total{reason="5xx"}+=3`. (b) Server emits SSE `data:{type:error,error_code:LLM_STREAM_INTERRUPTED}` per B6; closes cleanly. |
...
App-level errors (-32000..-32099) carry `data.error_code` matching the `HttpErrorCode` catalog (§4.1.2) so operators correlate JSON-RPC errors with HTTP errors.
...
- **S61/S62/S63 mcp errors** — Unknown method (e.g. `resources/list`) → `-32601`. Unknown tool name in `tools/call` → `-32602` `MCP_TOOL_NOT_FOUND`. Missing required `query` in arguments → `-32602` `MCP_TOOL_INPUT_INVALID`.
...
**`POST /ingest/v1` validation order:** discriminator-shape (422) → `mime_type` in allow-list (415) → inline `len(content.encode("utf-8")) ≤ INGEST_INLINE_MAX_BYTES` / file HEAD-probe size ≤ `INGEST_FILE_MAX_BYTES` (413) → `minio_site` resolved (422 `INGEST_MINIO_SITE_UNKNOWN`) → file HEAD-probe exists (422 `INGEST_OBJECT_NOT_FOUND`). For `POST /ingest/v1/upload`: magic-byte signature check for binary MIMEs (docx/pptx/pdf) → 415 `INGEST_MAGIC_MISMATCH`. Worker-side guards: DOCX/PPTX zip preflight → 413 `INGEST_ARCHIVE_UNSAFE` persisted as `documents.error_code`; PDF page-count cap → 413 `INGEST_PDF_TOO_MANY_PAGES` likewise. Every guard rejection increments `ragent_ingest_rejected_total{reason}` (T-SEC.7).

| Method | Path | P1 Auth | Request | Response |
|---|---|---|---|---|
| POST   | `/ingest/v1`               | `X-User-Id` | **JSON** (`ingest_type ∈ {inline, file}`; see §3.1 schema) | `202 { task_id }` — `task_id` **is** the `document_id`. |
...
|---|---|---|---|:---:|
| **Ingest** | `_IdempotencyClean (delete_by_document_id + fan_out_delete) → _TextLoader → _MimeAwareSplitter {text/plain → DocumentSplitter \| text/markdown → _MarkdownASTSplitter \| text/html → _HtmlASTSplitter \| docx → _DocxASTSplitter \| pptx → _PptxASTSplitter \| else → _RaiseUnroutable} → DocumentJoiner → _BudgetChunker (1000/1500/100) → DocumentEmbedder (bge-m3, batch=32) → DocumentWriter (ES chunks_v1)` + `PluginRegistry.fan_out (per-plugin 60 s)` | Embedder 30 s/batch · ES bulk 60 s · MinIO get 30 s · plugin 60 s · aggregate `PIPELINE_TIMEOUT_SECONDS` (1800) | `tests/integration/test_ingest_pipeline.py` | **P1** sync |
...
| `MINIO_SITES`                         | (required)       | v2: JSON list of `{name, endpoint, access_key, secret_key, bucket, secure?, read_only?}`. Must include `name="__default__"` (inline ingest). Supersedes legacy single-site vars (`MINIO_ENDPOINT`, `MINIO_ACCESS_KEY`, `MINIO_SECRET_KEY`, `MINIO_SECURE`, `MINIO_BUCKET` — all removed in v2). |
...
**raw_content / content:** `raw_content` (`index: false`, _source-only) = original byte slice from splitter; `content` = normalized BM25 input. LLM context and `sources[].excerpt` use `raw_content` (`content` fallback for pre-v2 chunks). `source_url` stored as `keyword` for citation rendering.
