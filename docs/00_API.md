...
- `POST /chatagent/v3/session/continue` — body `{session}`; ragent enriches the upstream payload to `{session, apName, user}` (`apName` from config, `user` from the resolved identity — never the client) before `POST`ing to `${CHATAGENT_SESSION_API_URL}/continue`; same error mapping as the other session routes (`502 CHATAGENT_UPSTREAM_ERROR` / `504 CHATAGENT_TIMEOUT`).
**Realtime status (NATS, not an HTTP route):** when `NATS_SERVERS` is set, ragent publishes status transitions to the per-user subject `session.<user_id>.status` — `{session, running:true}` (run starts), `{session, running:false, hasNewReply:true}` (finishes with a new reply), `{session, running:false}` (finishes without one — `RUN_ERROR`/cancelled-only; `hasNewReply` omitted so an existing dot isn't wiped), `{session, hasNewReply:false}` (client marks read via `POST /session/read`). Deltas are partial — each event carries only the fields that transition changed. The frontend subscribes over its own NATS connection and merges deltas onto the `sessionList` snapshot. NATS core pub/sub is lossy, so the client **must re-fetch `sessionList` on (re)connect/error** to re-sync (the snapshot is the durable truth; the publish is best-effort). NATS unset → list is snapshot-only.
