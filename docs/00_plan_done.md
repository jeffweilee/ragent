...
## Track T-REDIS-AUTH — Redis Sentinel Password Support

> Source: 2026-08-08 設計討論。目標：讓 sentinel 模式能連到設有 `requirepass` 的 Redis/Sentinel 叢集。
> 目前四個呼叫點(`bootstrap/broker.py`、`clients/rate_limiter.py`、`clients/chat_stream_store.py`、`clients/pat_cache.py`)
> 建構 `Sentinel(...)` 時均未傳 `password=` / `sentinel_kwargs=`，部署有密碼的叢集時會直接收到
> `NOAUTH Authentication required`。方案 A：抽取共用純函式 `parse_sentinel_hosts()` 到 `utility/env.py`，
> 在各 `from_env()` 讀取新增的兩個 env var；standalone 密碼不受影響（走 URL）。

**Counter: 完成 2 / 未完成 0 / descope 0**

| # | Category | Task | Status | Owner |
|---|---|---|:---:|---|
| T-REDIS-AUTH.1 | Structural | • **Achieve:** 抽取 `parse_sentinel_hosts(hosts_raw: str) -> list[tuple[str, int]]` 純函式至 `utility/env.py`；四個呼叫點改用此函式取代各自的 `rsplit` 邏輯。行為不變，測試全綠。<br>• **Deliver:** `src/ragent/utility/env.py` (`parse_sentinel_hosts`); `tests/unit/test_utility_env.py` (新 host 解析測試：正常、多 host、空白過濾、格式錯誤 exit); 四個呼叫點 import 並使用新函式。<br>• **Success criteria:** 現有所有 sentinel/standalone 測試仍全綠；`parse_sentinel_hosts` 單元測試覆蓋正常 + edge case。 | [x] | Dev |
| T-REDIS-AUTH.2 | Behavioral | • **Achieve:** 新增 `REDIS_SENTINEL_MASTER_PASSWORD` 與 `REDIS_SENTINEL_PASSWORD` env var；四個呼叫點讀取後傳入 `Sentinel(password=..., sentinel_kwargs={"password": ...})` 及 `ListQueueSentinelBroker(password=..., sentinel_kwargs=...)`；留空則行為與現在相同。<br>• **Deliver:** `.env.example` (2 個新變數 + 註解); `docs/spec/env_vars.md` (2 個新 var 行); 四個呼叫點; `tests/unit/test_broker_topology.py` + `test_rate_limiter.py` + `test_chat_stream_store.py` + `test_pat_cache.py`(各新增密碼注入測試：有密碼傳入、留空不傳、sentinel/master 各自獨立)。<br>• **Success criteria:** 有密碼時 `Sentinel`/`ListQueueSentinelBroker` 以正確 kwargs 建構；留空時與現有行為一致；env drift 測試全綠。 | [x] | Dev |

---

## Track T-RG — Redis Resilience / Weak-Dependency Guard

> Source: 2026-08-10 journal row "Weak-Dep Latency Bound" — a half-dead Redis
> stalled the whole API because fail-soft bounded outcome, not latency. Bounds
> socket/connect timeouts, circuit-breaks every Redis client, and defers
> ingest dispatch instead of failing it when the broker is unreachable.

**Counter: 完成 8 / 未完成 0 / descope 0**

| # | Category | Task | Status | Owner |
|---|---|---|:---:|---|
| T-RG.1 | Red+Green | • **Achieve:** per-client Redis circuit breaker — `RedisCircuit(name, failure_threshold, cooldown_seconds, clock)` with closed/open/half-open; open short-circuits to `UNAVAILABLE` with **zero I/O**; `WatchError` re-raised (healthy-server outcome, must not trip); non-`RedisError` propagates. Metrics `ragent_redis_circuit_state{client}` / `ragent_redis_circuit_trips_total{client}`.<br>• **Deliver:** `src/ragent/clients/redis_guard.py`; `bootstrap/metrics.py`; `tests/unit/test_redis_guard.py`.<br>• **Success criteria:** trips after threshold; open costs no call; failed half-open probe restarts the cooldown; a success resets the failure run. | [x] | Dev |
| T-RG.2 | Red+Green | • **Achieve:** bounded socket budget on every Redis client and the TaskIQ broker — `REDIS_CONNECT_TIMEOUT_SECONDS` (0.25) / `REDIS_SOCKET_TIMEOUT_SECONDS` (1), standalone **and** Sentinel, with the budget reaching `sentinel_kwargs` so master discovery is bounded too. Shared `build_redis_client()` collapses the three duplicated Sentinel builders.<br>• **Deliver:** `clients/redis_guard.py`; `clients/{pat_cache,rate_limiter,chat_stream_store}.py`; `bootstrap/broker.py`; `tests/unit/test_redis_timeouts.py`.<br>• **Success criteria:** all three clients bounded in both topologies; discovery connections bounded; operator-tunable. | [x] | Dev |
| T-RG.3 | Red+Green | • **Achieve:** route every op of `PatCache` / `RateLimiter` / `ChatStreamStore` through its own breaker with a documented degraded value; `ChatStreamStore.append`/`mark_done` become fail-soft (were the only two that could kill the producer thread mid-run); `acquire_refresh_lock` reports `UNAVAILABLE` distinctly from "held".<br>• **Deliver:** the three clients; `tests/unit/test_redis_clients_degrade.py`.<br>• **Success criteria:** correct degraded default per op; zero I/O once open; each client's breaker independent; `put` still honours the revocation tombstone. | [x] | Dev |
| T-RG.4 | Red+Green | • **Achieve:** `PatService._refresh` must not poll for a lock nobody can take — `UNAVAILABLE` breaks the poll loop immediately (was 10 × 0.5 s sleeps **plus** a blocking Redis call per attempt, on every request) and refreshes unserialised.<br>• **Deliver:** `services/pat_service.py`; `tests/unit/test_pat_service_refresh.py`.<br>• **Success criteria:** zero sleeps when Redis is unavailable; polling preserved when the lock is genuinely held. | [x] | Dev |
| T-RG.5 | Red+Green | • **Achieve:** an unreachable broker defers instead of failing ingest — `TaskiqDispatcher` raises typed `TaskDispatchUnavailable`; `IngestService._dispatch` logs `ingest.dispatch_deferred`, counts `ragent_ingest_dispatch_deferred_total`, keeps the 202. `TaskNotRegisteredError` still escapes.<br>• **Deliver:** `bootstrap/dispatcher.py`; `bootstrap/metrics.py`; `services/ingest_service.py`; `tests/unit/test_ingest_dispatch_degrade.py`.<br>• **Success criteria:** create/rerun/batch_rerun/upload all survive a dead broker; unregistered task still 500s; per-site log event names preserved. | [x] | Dev |
| T-RG.6 | Red+Green | • **Achieve:** ES + Redis become **advisory** probes — `/readyz` returns `200 {"status":"degraded","degraded":[…]}` instead of 503 (a 503 pulls the Pod from the k8s Service at `failureThreshold: 3`), `/startupz` latches, `_check_infra_ready` logs `api.startup.degraded` instead of aborting boot, and `auto_init` survives an unreachable ES. MariaDB + MinIO still gate.<br>• **Deliver:** `routers/health_probes.py` (`ADVISORY_PROBES`); `routers/health.py`; `bootstrap/app.py`; `bootstrap/init_schema.py`; `tests/unit/test_probes_advisory.py`; `docs/00_spec.md`.<br>• **Success criteria:** advisory failure never 503s or aborts boot; required failure still does and outranks advisory; degradation still visible via probe metrics. | [x] | Dev |
| T-RG.7 | Red+Green | • **Achieve:** address Codex review on PR #246 — (a) `RedisCircuit` half-open admits exactly ONE probe per cooldown (reservation under a lock; the bare time check let every concurrent caller through, each paying a socket timeout); (b) `auto_init` scopes ES degradation to availability only (`URLError`/timeout/`ConnectionError`/5xx), so malformed resources and 4xx still abort boot; (c) `_dispatch` returns success and `batch_rerun` reports a separate `deferred` count instead of inflating `queued`.<br>• **Deliver:** `clients/redis_guard.py`; `bootstrap/init_schema.py`; `services/ingest_service.py`; `routers/admin_ops.py`; `tests/unit/test_redis_guard.py`, `test_probes_advisory.py`, `test_ingest_dispatch_degrade.py`, `test_admin_ops_router.py`, `test_ingest_service_batch_rerun.py`; `docs/00_spec.md`, `docs/00_API.md`.<br>• **Success criteria:** 8 concurrent callers yield exactly 1 probe and the next window still opens; malformed JSON / 4xx propagate while URLError / 5xx degrade; `queued + deferred + skipped` accounts for every listed document. | [x] | Dev |
| T-RG.8 | Red+Green | • **Achieve:** fix the worker outage introduced by T-RG.2 — a `socket_timeout` on the TaskIQ broker killed `ListQueueBroker.listen()`'s indefinite `brpop()` wait every second with a `redis.TimeoutError` (not a `ConnectionError`, so listen()'s handler missed it), stopping consumption entirely and stranding every document at UPLOADED. `connection_timeouts(blocking=True)` now omits the read timeout for the broker's master connections while keeping the connect timeout; Sentinel discovery keeps the full budget.<br>• **Deliver:** `clients/redis_guard.py`; `bootstrap/broker.py`; `tests/unit/test_redis_timeouts.py`; `docs/00_spec.md`; `docs/00_journal.md`.<br>• **Success criteria:** broker has `socket_connect_timeout` but no `socket_timeout` in both topologies; Sentinel discovery keeps both; the request-path clients still keep both. | [x] | Dev |

---

## Track T-RETRY — Remove application-level retry from upstream clients

> Source: 2026-08-10 design session (下游服務失效盤點). Every AI client wraps its
> upstream call in `for attempt in range(3): try: … except Exception`, which
> retries **timeouts** (the call already burned its full budget) and **4xx**
> (the upstream said no — repeating the question cannot change the answer).
> Because the budgets are per-layer and nothing shares a deadline, a chat
> request could spend ~490 s retrying before the caller sees an error, and the
> retry traffic lands on an upstream that is already failing.
>
> **Decision: no application-level retry.** A failed call surfaces immediately
> as the typed `UpstreamServiceError` / `UpstreamTimeoutError` it already maps
> to. The one retained retry is at the **connection layer**
> (`httpx.HTTPTransport(retries=2)`), which covers only failures where the
> connection was never established — the server did no work, so retrying adds
> no load and duplicates no side effect. `storage/minio_registry.py` (permanent
> S3 errors re-raised, transient ones retried) and `docs/00_spec.md §290`
> ("a 4xx … deployment defects no retry heals") already hold this position;
> this track brings the AI clients in line.
>
> **Accepted cost:** on the ingest path a pipeline failure terminalises to
> `FAILED`, and the reconciler only redispatches stale `UPLOADED`/`PENDING` —
> so a transient embedding blip now needs `POST /ops/v1/retry` instead of
> healing inline. The connection-layer retry covers the most common such blip
> (pod rollover / RST), which is why it is kept.

**Counter: 完成 6 / 未完成 0 / descope 0**

| # | Category | Task | Status | Owner |
|---|---|---|:---:|---|
| T-RETRY.1 | Red+Green | • **Achieve:** `EmbeddingClient` / `RerankClient` / `LLMClient` issue **exactly one** upstream call per invocation — the five `for attempt in range(3)` loops and the injected `sleep` seam are deleted; `classify_upstream_error` mapping, the `LLMStreamInterruptedError` escape hatch, and every typed `error_code` stay byte-identical. `llm.complete()`'s empty-content `ValueError` and `embedding._validate_vectors`'s poison-vector guard now fail on the first occurrence (operator decision 2026-08-10).<br>• **Deliver:** `clients/embedding.py`, `clients/rerank.py`, `clients/llm.py`; `tests/unit/test_embedding_client.py`, `test_rerank_client.py`, `test_llm_client.py`, `test_llm_client_chat.py`, `test_clients_tracing.py`.<br>• **Success criteria:** every failure test asserts `http.post.call_count == 1`; no `sleep` kwarg remains on any client ctor; error type / `error_code` / `http_status` per client unchanged. | [x] | Dev |
| T-RETRY.2 | Behavioral | • **Achieve:** the shared `http` and `auth_http` clients retry **connection establishment only** — `httpx.HTTPTransport(retries=2)`, which by construction cannot re-issue a request that reached the server.<br>• **Deliver:** `bootstrap/composition.py`; `tests/unit/test_composition_http_transport.py`.<br>• **Success criteria:** both clients carry a transport with `retries=2`; a response-level failure (4xx/5xx/timeout) still yields exactly one request. | [x] | Dev |
| T-RETRY.3 | Red+Green | • **Achieve:** `PatService._do_refresh` makes **one** refresh attempt — the 429 exp-backoff loop, `max_retries` / `backoff_base_seconds` / `sleeper` ctor params, and `PAT_REFRESH_MAX_RETRIES` / `PAT_REFRESH_BACKOFF_SECONDS` are gone; a 429 or transient failure raises `PatRefreshExhausted` (503) on the first hit with the PAT still `active`. `PatCache.tombstone_ttl_for` loses its retry terms and derives from `PAT_REFRESH_TIMEOUT_SECONDS` **plus the lock-poll budget** (`LOCK_POLL_ATTEMPTS × LOCK_POLL_INTERVAL_SECONDS`, exported from `pat_service` so composition stays the single reader): the longest path from "refresh started" to `cache.put` belongs to a request that waited out the full poll and then refreshed itself, and the old formula omitted that term because the retry budget happened to dwarf it. Side effect: `REDIS_PAT_LOCK_TTL_SECONDS` (45 s) now genuinely exceeds the worst-case refresh (30 s) instead of only the typical one.<br>• **Deliver:** `services/pat_service.py`, `clients/pat_cache.py`, `bootstrap/composition.py`; `docs/spec/pat.md §3.2/§5`, `docs/spec/env_vars.md`, `.env.example`; `tests/unit/test_pat_service_refresh.py`, `test_pat_cache.py`, `test_composition_pat_wiring.py`.<br>• **Success criteria:** one refresh call per resolve; 401 still invalidates, 400 still 500s, 429 no longer retries; tombstone TTL still outlasts the worst-case in-flight refresh; env drift test green. | [x] | Dev |
| T-RETRY.4 | Behavioral | • **Achieve:** the Elasticsearch client's retry policy stops being an invisible library default — `max_retries=0` is set explicitly at construction (`elastic_transport` 8.17.1 defaults to `max_retries=3, retry_on_status=(429, 502, 503, 504)`, a retry layer no call site can see).<br>• **Deliver:** `bootstrap/composition.py`; `tests/unit/test_composition_es_retry.py`.<br>• **Success criteria:** the constructed client reports `max_retries == 0`; a verified-against-version comment sits next to the kwarg per `00_rule.md §Third-Party`. | [x] | Dev |
| T-RETRY.5 | Structural | • **Achieve:** the contract docs stop promising retries that no longer exist — and stop promising one that never did (`00_rule_third_party_api.md §120` documents "Retry: 3x @ 1.0s backoff" for the auth service, but `clients/auth.py::_refresh` has no loop at all).<br>• **Deliver:** `docs/00_rule_third_party_api.md` (four `Retry:` lines), `docs/00_spec.md §461`, `docs/spec/decision_log.md` (no-retry decision + rationale + accepted cost), `docs/00_domain_map.md §2.7` (Clients 責任 drops "retry").<br>• **Success criteria:** no doc claims an application-level retry; the decision log records why timeouts and 4xx are not retried and why the connection layer is. | [x] | Dev |
| T-RETRY.6 | Red+Green | • **Achieve:** address the automated review on PR #247. **(a)** the retrying transport silently disabled every environment proxy — httpx computes `allow_env_proxies = trust_env and transport is None` (0.28.1), so a proxy-only deployment lost *all* outbound calls (AI, auth, PAT, chatagent, brain); `_http_client` rebuilds the mounts from httpx's own `get_environment_proxies()` so `NO_PROXY` semantics stay httpx's to define. **(b)** `ElasticsearchDocumentStore` builds its own ES client — the one the chat retrieval path actually searches through — so it inherited `max_retries=3` while only the standalone client was fixed; both now share `_ES_RETRY_KWARGS`. **(c)** `{"usage": null}` became an unhandled `AttributeError` (500) once T-RETRY.1 moved the `usage_raw.get(...)` reads outside the try, where they had previously been wrapped as a typed 502.<br>• **Deliver:** `bootstrap/composition.py` (`_http_client` mounts, `_document_store`, `_ES_RETRY_KWARGS`), `clients/llm.py`; `tests/unit/test_composition_http_transport.py`, `test_composition_es_retry.py`, `test_llm_client_chat.py`; `docs/00_journal.md` (Architecture — Transport Config Ownership).<br>• **Success criteria:** proxy routing is asserted **differentially** against a stock `httpx.Client` built under the same env (HTTP_PROXY / HTTPS_PROXY / ALL_PROXY / NO_PROXY), not against hand-written patterns; the document store's kwarg is asserted to reach `Elasticsearch(...)` rather than merely sit on `_kwargs`; an explicit-null `usage` returns zero counts instead of raising.<br>• **Landed:** httpx forwards `retries` only to the direct `ConnectionPool`, never to `httpcore.HTTPProxy` — so behind a proxy there is no connection retry at all. Not a regression (the pre-T-RETRY client had `retries=0` everywhere), but the retained retry is a direct-connection benefit only; pinned with a test so an httpx upgrade that starts honouring it surfaces here. | [x] | Dev |
