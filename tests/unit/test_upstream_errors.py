...
def test_upstream_service_error_default_attributes() -> None:
    ...
    assert exc.error_code == HttpErrorCode.UPSTREAM_ERROR
    assert exc.http_status == 502
    assert "embed boom" in str(exc)


def test_upstream_service_error_custom_error_code() -> None:
    exc = UpstreamServiceError("boom", service="embedding", error_code=HttpErrorCode.EMBEDDER_ERROR)
    assert exc.error_code == HttpErrorCode.EMBEDDER_ERROR
    assert exc.http_status == 502
...
def test_upstream_timeout_error_defaults_to_504() -> None:
    ...
    assert exc.error_code == HttpErrorCode.UPSTREAM_TIMEOUT
    assert isinstance(exc, UpstreamServiceError)


def test_upstream_timeout_error_custom_error_code() -> None:
    exc = UpstreamTimeoutError("slow", service="llm", error_code=HttpErrorCode.LLM_TIMEOUT)
    assert exc.http_status == 504
    assert exc.error_code == HttpErrorCode.LLM_TIMEOUT
