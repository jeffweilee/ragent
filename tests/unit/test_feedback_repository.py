"""T-FB.4 — FeedbackRepository.upsert: idempotent on (user_id, request_id, source_id)."""
...
async def test_upsert_executes_insert_on_duplicate_key_update():
    ...
    repo = FeedbackRepository(engine)
    await repo.upsert(request_id="REQ", user_id="alice", source_id="S1", vote=1, reason=None)
...
async def test_upsert_accepts_null_reason():
    engine, conn = _mock_engine()
    repo = FeedbackRepository(engine)
    await repo.upsert(request_id="R", user_id="u", source_id="s", vote=-1, reason=None)
...
async def test_upsert_rejects_vote_outside_unit():
    ...
    with pytest.raises(ValueError):
        await repo.upsert(request_id="R", user_id="u", source_id="s", vote=0, reason=None)
    with pytest.raises(ValueError):
        await repo.upsert(request_id="R", user_id="u", source_id="s", vote=2, reason=None)
...
async def test_upsert_position_shown_defaults_to_none():
    engine, conn = _mock_engine()
    repo = FeedbackRepository(engine)
    await repo.upsert(request_id="R", user_id="u", source_id="s", vote=1, reason="other")
