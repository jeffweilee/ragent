...
# --- T-PAT.29: the management surface resolves NO PAT -------------------------


def test_proxy_attaches_no_pat() -> None:
    """This surface makes no drive call, so it resolves no PAT — only the run
    path does. Rationale (a side-effecting resolve on 33 read-only routes is what
    turns a redis outage into a PAT outage): `docs/spec/pat.md` §7 / B65."""
    seen: dict = {}
...
def test_proxy_router_takes_no_pat_service() -> None:
    # The injection seam is gone on purpose, so re-attaching a PAT to this
    # surface cannot happen by wiring alone — it takes a signature change.
